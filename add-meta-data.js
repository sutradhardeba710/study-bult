  ...
  try {
    ...
    for (const [type, items] of Object.entries(sampleData)) {
      ...
      if (snapshot.empty) {
        ...
        console.log(`${type} collection has ${snapshot.size} items`);
        
        // Check for missing items
        const existingNames = snapshot.docs.map(doc => doc.data().name);
        const missingItems = items.filter(item => !existingNames.includes(item.name));
        
        if (missingItems.length > 0) {
          console.log(`Adding ${missingItems.length} missing items to ${type}...`);
          ...
          console.log(`No missing items in ${type}`);
        }
      }
    }

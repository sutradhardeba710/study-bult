...
/**
 * Cloud Function to clean up storage file when a paper document is deleted
 */
        ...
        try {
            ...
            if (storagePath) {
                try {
                    ...
                    functions.logger.info(`Deleted storage file for paper ${context.params.paperId} at ${storagePath}`);
                    await logRef.update({
                        status: 'success',
                    ...
                    // Ignore "not found" errors, as it might have been deleted by the client
                    if (error.code === 404) {
                        functions.logger.info(`File already deleted or not found at ${storagePath}`);
                        await logRef.update({
                            status: 'success',
                        ...
                        functions.logger.warn(`Failed to delete storage file at ${storagePath}:`, error);
                        await logRef.update({
                            status: 'error',
                            error: error.message || 'Storage deletion failed',
                            completedAt: admin.firestore.FieldValue.serverTimestamp()
                        });
                    }
                }
            } else {
                functions.logger.warn(`No storagePath found for deleted paper ${context.params.paperId}`);
                await logRef.update({
                    status: 'warning',
                    message: 'No storagePath found in paper document',
                    completedAt: admin.firestore.FieldValue.serverTimestamp()
                });
            }
        } catch (error: any) {
            functions.logger.error('Error in onPaperDeleted:', error);
            // Try to log the error if possible, but don't crash
            try {
                await logRef.set({
                    type: 'paper_deletion_error',
                    error: error.message,
                    timestamp: admin.firestore.FieldValue.serverTimestamp()
                });
            } catch (e) {
